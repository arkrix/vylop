...
const Home = () => {
    ...
    const joinRoom = async (overrideMode) => {
        if (!joinRoomId.trim() && !overrideMode) {
            toast.error('ROOM ID is required');
            return;
        }

        const activeRoomId = joinRoomId.trim() || uuidv4();
        const finalRoomName = joinRoomName.trim() ? joinRoomName : (overrideMode ? `${overrideMode} Session` : "Joined Workspace");

        try {
            await axios.post(
                `${API_BASE_URL}/api/workspace/${activeRoomId}/register?username=${encodeURIComponent(username)}&roomName=${encodeURIComponent(finalRoomName)}`
            );
        } catch (error) {
            console.warn("Could not pre-register room name:", error);
        }
        ...
        navigate(`/room/${activeRoomId}`, {
            state: {
                username,
                roomName: finalRoomName,
                mode: overrideMode || 'editor'
            },
        });
    };

    const handleHostInterview = () => {
        const interviewId = uuidv4();
        toast.success('Initializing Technical Interview Session...', { icon: '💼' });
        navigate(`/room/${interviewId}`, {
            state: {
                username,
                roomName: `Interview Session - ${username}`,
                mode: 'interview'
            }
        });
    };
    ...
    const handleJoinCompetition = () => {
        toast.success('Connecting to Arena...', { icon: '🏆' });
        ...
        <div className="homePageWrapper">
            ...
            {workspaceToDelete && (
                <div className="modal-overlay" onClick={() => setWorkspaceToDelete(null)}>
                    <div className="custom-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p style={{ color: 'var(--text-muted, #8b949e)', fontSize: '0.9rem', margin: '5px 0 20px 0', lineHeight: '1.5' }}>
                    ...
                    <div className="custom-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            <input
                                type="text"
                                className="modern-input"
                                placeholder="ROOM ID (e.g. 123e4567...)"
                                value={joinRoomId}
                            ...
                            <input
                                type="text"
                                className="modern-input"
                                placeholder="OPTIONAL: Local Room Name"
                                value={joinRoomName}
                        ...
                        <div className="profile-dropdown-menu">
                            ...
                            <button className="menu-item logout-btn" onClick={handleLogout}>
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M10 21H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 11l4 4-4 4M20 15H10"/>
                                </svg>
                                Logout
                            </button>
                        </div>
            ...
            <div style={{ padding: '0 20px', maxWidth: '1200px', margin: '0 auto', width: '100%', boxSizing: 'border-box', flex: 1 }}>
                ...
                <div style={{ marginBottom: '50px', textAlign: 'center' }}>
                    <h1 style={{ fontSize: '2.8rem', marginBottom: '15px', letterSpacing: '-0.5px', color: '#fff' }}>
                        Welcome back to <span style={{ color: '#58a6ff' }}>Vylop</span>.
                    </h1>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '60px' }}>
                    ...
                    {/* Card 2: Interview */}
                    <div style={{ background: 'linear-gradient(145deg, rgba(210, 153, 34, 0.1) 0%, rgba(33, 38, 45, 0.4) 100%)', borderRadius: '16px', padding: '30px', border: '1px solid rgba(210, 153, 34, 0.2)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <h2 style={{ fontSize: '1.4rem', margin: 0, color: '#fff' }}>Host Interview</h2>
                        <p style={{ color: '#8b949e', fontSize: '0.9rem', lineHeight: '1.5', flex: 1, margin: 0 }}>
                            Host a secure, structured technical interview. Includes hidden test cases, host-only controls, and candidate evaluation tools.
                        </p>
                        <button className="btn btn-secondary" style={{ width: '100%', marginTop: '10px', borderColor: 'rgba(210, 153, 34, 0.4)', color: '#d29922' }} onClick={handleHostInterview}>
                            Configure Interview
                        </button>
                    </div>
                    ...
                    <div style={{ background: 'linear-gradient(145deg, rgba(137, 87, 229, 0.1) 0%, rgba(33, 38, 45, 0.4) 100%)', borderRadius: '16px', padding: '30px', border: '1px solid rgba(137, 87, 229, 0.2)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <p style={{ color: '#8b949e', fontSize: '0.9rem', lineHeight: '1.5', flex: 1, margin: 0 }}>
                            Enter a competitive coding arena. Strict timers, automated algorithmic scoring, and live leaderboards.
                        </p>
                        <button className="btn btn-secondary" style={{ width: '100%', marginTop: '10px', borderColor: 'rgba(137, 87, 229, 0.4)', color: '#8957e5' }} onClick={handleJoinCompetition}>
                            View Active Arenas
                        </button>
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', margin: '40px 0' }}>
                    ...
                    <span style={{ padding: '0 20px', color: '#8b949e', fontSize: '0.85rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>
                        Your Saved Cloud Workspaces
                    </span>
                    <div style={{ flex: 1, height: '1px', background: '#30363d' }}></div>
                </div>
                ...
                {isLoadingRooms ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '40px', backgroundColor: 'rgba(255,255,255,0.01)', borderRadius: '12px', border: '1px dashed #30363d' }}>
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" style={{ color: '#8b949e', marginBottom: '15px' }}>
